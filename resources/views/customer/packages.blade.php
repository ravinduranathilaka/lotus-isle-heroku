                ...
                @foreach ($packages as $package)
                    <tr>
                        ...
                        <td class="border px-4 py-2 text-center">
                            <button onclick="openOrderModal({{ $package->id }}, '{{ $package->name }}')" class="bg-green-600 hover:bg-green-700 text-red font-bold px-4 py-2 rounded shadow">
                                Order
                            </button>
                        </td>
                    </tr>
    ...
    <div id="orderModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
            ...
            <form method="POST" action="{{ route('customer.orders.store') }}">
                ...
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-red font-semibold py-2 rounded">
                    Confirm Order
                </button>
            </form>
        </div>
